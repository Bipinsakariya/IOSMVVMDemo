...
extension CustomTextField {
    func hightLighttextField() {
       self.borderColor = AppColor.orangeColor
       self.borderWidth = 2
       self.textColor = AppColor.orangeColor
    }
}
...
class EnterCodeVC: BaseViewController, BaseDataSources {
    ...
    func setUpView() {
        ...
        self.subheadingLabel.attributedText = ("Enter the code we sent to \n\(code) \(number.toPhoneNumber())" as NSString).addBoldText(boldPartsOfString: ["\(code) \(number.toPhoneNumber())"], font: AppFont.MuseoSansCyrl_700(fontSize: 17), boldFont: AppFont.MuseoSansCyrl_900(fontSize: 17), boldFontcolor: UIColor.white)
